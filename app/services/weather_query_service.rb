class WeatherQueryService
  ...
  def weather_by_zipcode
    weather_url = "#{CURRENT_WEATHER_BASE_URL}?zip=#{zipcode},us&appid=#{api_key}&units=imperial"
    JSON.parse(Net::HTTP.get(URI(weather_url)))
  end

  def weather_by_lat_long
    weather_url = "#{CURRENT_WEATHER_BASE_URL}?lat=#{lat_long[0]}&lon=#{lat_long[1]}&appid=#{api_key}&units=imperial"
    JSON.parse(Net::HTTP.get(URI(weather_url)))
  end
end